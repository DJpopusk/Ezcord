...
function buildAppUrl() {
    ...
    if (window.location.origin === 'null' || !window.location.origin) {
        const base = window.location.href.replace(/login\.html.*$/, '');
        return base + appFilename;
    }
    const basePath = window.location.pathname.replace(/[^/]+$/, '');
    return `${window.location.origin}${basePath}${appFilename}`;
}
...
function navigateToApp() {
    ...
    // Safety fallback in case navigation was blocked (some embedded contexts)
    setTimeout(() => {
